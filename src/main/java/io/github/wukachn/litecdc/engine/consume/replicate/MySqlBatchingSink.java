    ...
    if (columns != null) {
      ...
      while (afterIter.hasNext()) {
        ...
        switch (column.getDetails().getSqlType()) {
          ...
          case Types.TINYINT, Types.SMALLINT, Types.INTEGER, Types.BIGINT -> stmt.setLong(
              idx, (long) column.getValue());
          case Types.FLOAT, Types.DOUBLE, Types.REAL -> stmt.setDouble(
              idx, (Double) column.getValue());
